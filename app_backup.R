...
sliderInputdate <- function(id,setvalue){
  sliderInput(id,label = "",min = 1,max = 31,value = setvalue)
}
...
checkboxGroupInputhosp <- function(id, setlabel, choice, selected = c("CGH", "SGH", "SKH")){
  setlabel <- switch(
    id,
    "SH" = "Singhealth",
  ...
  checkboxGroupInput(id, label = setlabel, choices = choices, selected = selected)
}
